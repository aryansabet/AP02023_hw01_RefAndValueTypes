...
namespace A6.Tests
{
    ...
    public class MemoryTestTests
    {
        ...
        {
            VerifySize<TypeOfSize5>(5);
            VerifySize<TypeOfSize22>(22);
            VerifySize<TypeOfSize125>(125);
            VerifySize<TypeOfSize1024>(1024);
            VerifySize<TypeOfSize32768>(32768);
        }
        ...
        public void StackDepth10Test()
        {
            ...
            {
                VerifyApproximateMatch(Marshal.SizeOf<TypeForMaxStackOfDepth10>(), 110000);
            }
            else
            {
        ...
        public void StackDepth100Test()
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                Console.WriteLine("Windows: ", recursionDepth);
                recursionDepth /= 16;
                Console.WriteLine("Linux: ", recursionDepth);
            }
        ...
        private static void VerifyApproximateMatch(int actual, int expected)
        {
            ...
            Assert.IsTrue(Math.Abs(actual - expected) <= (prec * expected), $"Actual:{actual} != Expected:{expected}");
        }
        ...
        private void VerifySize<_Type>(int expectedSize)
        {
            int actualSize = Marshal.SizeOf<_Type>();
